# Questões

Guia para revisão posterior das passagens anotadas.

### Tópicos para estudo (UD)
- Pesquisar na página geral do UD e na de latim do CIRCSE  a forma de marcação de dependências para pronomes relativos em orações relativas.


# Parágrafo 10 (Rep.6.10)

- Linha 213 (sent_id = cic-rep-6-10-C, ID 28 TOKEN quo): na oração relativa, de Homero…de quo… solebat, o quo possui como HEAD 32, isto é, o verbo solebat (linha 217) e é um “obl”, ou ele possui como HEAD 24, isto é, o nome que el retoma, Homero (linha 209) ?

# Parágrafo 12 (Rep.6.12)
- Linha 430  (sent_id = cic-rep-6-12-C, ID 19 TOKEN alter): head e deprel
- Linhas 458, 461, 463  (sent_id = cic-rep-6-12-D, ID 3, 6, 8 TOKEN te): deprel
- Linha 480  (sent_id = cic-rep-6-12-E, ID 11 TOKEN multa): features, head e deprel
- Linha 490  (sent_id = cic-rep-6-12-E, ID 21 TOKEN effugeris): head (15?16?)

# Parágrafo 13 (Rep.6.13)
- Linha 540  (sent_id = cic-rep-6-13-A, ID 19 TOKEN esse): head (0,23) deprel (root? cop?), TAG (verb? aux?); visto que o verbo possui sentido de existência, e não de qualidade
- Linha 555  (sent_id = cic-rep-6-13-B, ID 2 TOKEN est): head (0?), deprel (root?); visto que o verbo possui sentido de existência, e não de qualidade
- Linha 571  (sent_id = cic-rep-6-13-B, ID 18 TOKEN concilia): deprel 

